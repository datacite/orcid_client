...
describe OrcidClient::Notification, vcr: true do
  ...
  let(:samples_path) { "resources/notification_2.0/samples/" }
  ...
  describe 'schema' do
    it 'validates sample' do
      validation_errors = subject.schema.validate(samples_path + 'notification-permission-2.0.xml').map { |error| error.to_s }
